...
public class GenerateXP {
    ...
    public static void setParametersFluo(Experiment xp, boolean processing, boolean mutationHighBck, boolean measurements) {
        ...
        if (processing) {
            ...
            bacteria.setProcessingPipeline(new SegmentAndTrack(
                            ...
                            new PostFilter(new RemoveEndofChannelBacteria().setContactSidesProportion(0.5)).setDeleteMethod(2).setMergePolicy(PostFilter.MERGE_POLICY.MERGE_TRACKS_SIZE_COND), 
        ...
        if (measurements) {
            ...
            xp.addMeasurement(new GrowthRate().setFeature(new Size(), true).setSuffix("Area"));
            xp.addMeasurement(new GrowthRate().setFeature(new FeretMax(), true).setSuffix("Length"));
    ...
    public static void setParametersPhase(Experiment xp, boolean processing, boolean measurements) {
        ...
        if (processing) {
            ...
            bacteria.setProcessingPipeline(new SegmentAndTrack(
                        ...
                        new NormalizeTrack(1, true)
                            ...
                            new FeatureFilter(new Size(), 200, true, true), 
                            new BinaryClose(5),
                            new FillHoles2D()
                    ).addTrackPostFilters(
        ...
        if (measurements) {
            ...
            xp.addMeasurement(new GrowthRate().setFeature(new Size(), true).setSuffix("Area"));
            xp.addMeasurement(new GrowthRate().setFeature(new FeretMax(), true).setSuffix("Length"));
            xp.addMeasurement(new GrowthRate().setFeature(new SpineLength(), true).setSuffix("SpineLength"));
        }
    }
