...
public class MutationTracker implements TrackerSegmenter, MultiThreaded, ParameterSetupTracker, ToolTip {
    ...
    public void track(int structureIdx, List<StructureObject> parentTrack, boolean LQSpots) {
        ...
        DistanceComputationParameters distParams = new DistanceComputationParameters().setQualityThreshold(spotQualityThreshold).setGapDistancePenalty(gapPenalty).setAlternativeDistance(alternativeDistance).setAllowGCBetweenLQ(true).setMaxFrameDifference(maxGap);
