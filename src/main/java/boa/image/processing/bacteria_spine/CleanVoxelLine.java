...
public class CleanVoxelLine {
    ...
    public static Set<Voxel> cleanContour(Set<Voxel> contour) {
        return new CleanVoxelLine(contour, false).cleanContour();
    }
    /**
    ...
    private void keepOnlyLargestCluster() {
        ...
        if (clusters.size()>1) { 
            ...
            if (verbose) ImageWindowManagerFactory.showImage(draw(true).setName("neighbors keep largest cluster"));
            if (verbose) ImageWindowManagerFactory.showImage(draw(false).setName("labels keep largest cluster"));
        }
    }
    ...
    private boolean cleanContourJunction(Vertex junction) {
        ...
        switch (junction.connectedSegments.size()) {
            ...
            default: { 
                ...
                junction.relabel();
                if (verbose) {
                    ImageWindowManagerFactory.showImage(draw(true).setName("neigh after clean junction: >2 branch connected to junction #"+junction.label));
                    ImageWindowManagerFactory.showImage(draw(false).setName("labels after clean junction: >2 branch connected to junction #"+junction.label));
                }
                //if (verbose) throw new RuntimeException("LARGEST SHORTEST LOOP");
                return true;
            }
        }
